...
TIME_IN_BRIDGE_CARS = 1 #A car takes 1s crossing
TIME_IN_BRIDGE_PEDESTRIAN = 10 #A pedestrian takes 10s crossing
...
class Monitor():
	def __init__(self):
		...
		self.pass_N = Condition(self.mutex) #Condition variable for cars heading North
		self.pass_S = Condition(self.mutex) #Condition variable for cars heading South
		self.pass_P = Condition(self.mutex) #Condition variable for pedestrians
	...
	def can_N(self) -> bool:
		return self.n_south.value == 0 and self.n_ped.value == 0 and self.n_north.value == 0
		
	def can_S(self) -> bool:
		return self.n_north.value == 0 and self.n_ped.value == 0 and self.n_south.value == 0 
	...
	def leaves_car_N(self, cid) -> None:
		...
		if self.n_north.value == 0:
			...
			self.pass_S.notify()
			if self.waiting_S.value == 0 and self.waiting_P.value == 0:
				self.pass_N.notify()
		self.mutex.release()
	...
	def leaves_car_S(self, cid) -> None:
		...
		if self.n_south.value == 0:
			self.pass_P.notify_all()
			self.pass_N.notify()
			if self.waiting_N.value == 0 and self.waiting_P.value == 0:
				self.pass_S.notify()
		self.mutex.release()
...
def delay_car_north(factor = TIME_IN_BRIDGE_CARS) -> None:
    time.sleep(factor)

def delay_car_south(factor = TIME_IN_BRIDGE_CARS) -> None:
    time.sleep(factor)

def delay_pedestrian(factor = TIME_PED) -> None:
    time.sleep(factor)
